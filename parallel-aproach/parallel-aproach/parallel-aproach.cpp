...
extern "C" {
    __declspec(dllexport) void process_matrix(float* input_matrix, float* output_matrix,int*next_node_matrix, int n) {
        for (int k = 0; k < n; ++k) {
            #pragma omp parallel for num_threads(n) schedule(dynamic) collapse(2)
            for (int i = 0; i < n; ++i) {
                for (int j = 0; j < n; ++j) {
                    float v = input_matrix[i * n + k];
                    float val = v + input_matrix[k * n + j];
                    if (input_matrix[i * n + j] > val) {
                        input_matrix[i * n + j] = val;
